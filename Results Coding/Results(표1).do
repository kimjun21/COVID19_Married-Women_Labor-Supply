...
foreach y in 2016 2017 2018 2019 2020 2021 {
    ...
    * 트리밍 이후, 표본 개수 계산 (해당 연도에 대해 ipw가 생성된 관측치 수)
    count if year == `y' & ipw < .
    local trimmed_`y' = r(N)

    * 유지율 계산 및 출력
    local rate_`y' = 100 * `trimmed_`y'' / `total_`y''

    display ">> `y'년: 원표본 = `total_`y''명, 트리밍표본 = `trimmed_`y''명 → 유지율 = `rate_`y''%"
